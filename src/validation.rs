...
use crate::constants::{MAX_TOPIC_LENGTH, RESERVED_TOPICS};
...
mod tests {
    ...
    fn test_invalid_topics() {
        ...
        assert!(validate_topic("123topic").is_err());
        assert!(validate_topic("topic-name").is_err());
        assert!(validate_topic("topic name").is_err());
        assert!(validate_topic("all").is_err());
        assert!(validate_topic(&"a".repeat(51)).is_err());
    }
}
